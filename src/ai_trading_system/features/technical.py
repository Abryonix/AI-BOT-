...
def _money_flow_index(group: pd.DataFrame, window: int = 14) -> pd.Series:
    typical = (group["high"] + group["low"] + group["close"]) / 3
    raw_flow = typical * group["volume"]
    direction = np.sign(typical.diff()).fillna(0)
    positive = raw_flow.where(direction > 0, 0.0).rolling(window).sum()
    negative = raw_flow.where(direction < 0, 0.0).rolling(window).sum().abs()
...
def add_technical_features(
    frame: pd.DataFrame,
    windows: tuple[int, ...] = (5, 10, 14, 20, 50, 100, 200),
) -> pd.DataFrame:
    """Add technical, volatility, and volume features using only current/past rows."""
    ...
    for _, group in data.groupby("symbol", sort=False):
        ...
        g["return_1d"] = returns
        g["log_return_1d"] = log_returns
        g["intraday_return"] = close / open_ - 1
        g["overnight_gap"] = open_ / close.shift(1) - 1
        g["high_low_range_pct"] = (high - low) / close
        for window in windows:
            g[f"sma_{window}"] = close.rolling(window).mean()
            g[f"ema_{window}"] = close.ewm(span=window, adjust=False).mean()
            g[f"price_to_sma_{window}"] = close / g[f"sma_{window}"] - 1
            g[f"price_to_ema_{window}"] = close / g[f"ema_{window}"] - 1
            g[f"momentum_{window}"] = close / close.shift(window) - 1
            g[f"roc_{window}"] = close.pct_change(window)
            g[f"realized_volatility_{window}"] = log_returns.rolling(window).std() * np.sqrt(252)
            g[f"rolling_volatility_{window}"] = returns.rolling(window).std() * np.sqrt(252)
            g[f"downside_volatility_{window}"] = returns.clip(upper=0).rolling(window).std() * np.sqrt(252)
            g[f"volume_sma_{window}"] = volume.rolling(window).mean()
            g[f"volume_ratio_{window}"] = volume / g[f"volume_sma_{window}"]
            g[f"relative_volume_{window}"] = volume / volume.rolling(window).median()
            g[f"amihud_illiquidity_{window}"] = (returns.abs() / (close * volume).replace(0, np.nan)).rolling(window).mean()
        g["rsi_14"] = _rsi(close)
        ...
        g["bb_percent_b_20"] = (close - g["bb_lower_20"]) / (g["bb_upper_20"] - g["bb_lower_20"]).replace(0, np.nan)
        ...
        g["adx_14"] = _adx(g)
        g["mfi_14"] = _money_flow_index(g)
        g["obv"] = (np.sign(close.diff()).fillna(0) * volume).cumsum()
        g["obv_momentum_20"] = g["obv"].pct_change(20)
        ...
        g["chaikin_money_flow_20"] = (money_flow_mult.fillna(0) * volume).rolling(20).sum() / volume.rolling(20).sum()
        g["parkinson_vol_21"] = np.sqrt((1 / (4 * np.log(2))) * (np.log(high / low) ** 2).rolling(21).mean() * 252)
        g["garman_klass_vol_21"] = np.sqrt(
            (
                0.5 * np.log(high / low) ** 2
                - (2 * np.log(2) - 1) * np.log(close / open_) ** 2
            ).rolling(21).mean().clip(lower=0)
            * 252
        )
        g["sma_20_50_cross"] = g["sma_20"] / g["sma_50"] - 1
        g["ema_12_26_cross"] = ema12 / ema26 - 1
        out.append(g)
